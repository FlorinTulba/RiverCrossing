...
inline unsigned maxInvestigatedRaftConfigs(unsigned entsCount,
...
BOOST_AUTO_TEST_CASE(generateCombinations_usecases) {
  ...
  try {
    ...
    BOOST_CHECK(
        b = (size(combs) == combsCount(n, gsl::narrow_cast<size_t>(k))));
    if (b) {
  ...
  */
  class MissingVarExcHandler : public IValidatorExceptionHandler {
   public:
    ...
    MissingVarExcHandler& operator=(const MissingVarExcHandler&) = delete;
    MissingVarExcHandler& operator=(MissingVarExcHandler&&) noexcept = delete;
    ...
    */
    [[nodiscard]] boost::logic::tribool assess(
      ...
      if (!rc::makeNoexcept([this, &st] { return st.contains(luckyVarName); }))
        return outcome;
  ...
  try {  // vs depends now also on 'PreviousRaftLoad'
    ...
    BOOST_CHECK(static_cast<bool>(
        inprleh->assess(me1, InitialSymbolsTable())));  // idx is 0
    BOOST_CHECK(static_cast<bool>(
        inprleh->assess(me1, {{"CrossingIndex", 1.}})));  // true for idx <=1
...
BOOST_AUTO_TEST_CASE(movingConfigsManager_usecases) {
  ...
  sd.capacity = gsl::narrow_cast<unsigned>(entsCount) - 1U;

  vector<const MovingEntities*> configsForABank;

  try {
    {
      assert(static_cast<size_t>(sd.capacity) == entsCount - 1ULL);
      MovingConfigsManager mcm{sd, emptySt};

      const double investigatedRaftCombs{
          static_cast<double>(maxInvestigatedRaftConfigs(
              gsl::narrow_cast<unsigned>(entsCount),
              gsl::narrow_cast<unsigned>(entsCantRowCount), sd.capacity))};
      ...
      */
      ...
      BOOST_TEST(investigatedRaftCombs ==
                 static_cast<double>(size(mcm.allConfigs)));
...

    {
      ...
      BOOST_TEST(maxInvestigatedRaftCombs >=
                 static_cast<double>(size(mcm.allConfigs)));
...
BOOST_AUTO_TEST_CASE(solvingVariousScenarios) {
  ...
  try {
    ...
    assert(isgreaterequal(d.maxLoad, DBL_MAX));
