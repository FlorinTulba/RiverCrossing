...
BOOST_AUTO_TEST_CASE(typeNameRule_correctInput) {
  ...
  bool b{};
  const array inputs{
      string{"a"},     // typeName of exactly one alphabetic character
      string{"a1b-_"}  // typeName with all categories of accepted characters
  };
...
BOOST_AUTO_TEST_CASE(configurationsRule_correctInput) {
  ...
  BOOST_CHECK(b = (nullptr != v.at(0ULL)));
  if (b) {
  ...
  BOOST_CHECK(b = (nullptr != v.at(1ULL)));
  if (b) {
    BOOST_CHECK(b = (nullptr != (ic = dynamic_cast<const IdsConstraint*>(
                                     v.at(1ULL).get()))));
    if (b) {
  ...
  BOOST_CHECK(b = (nullptr != v.at(2ULL)));
  if (b) {
    BOOST_CHECK(b = (nullptr != (ic = dynamic_cast<const IdsConstraint*>(
                                     v.at(2ULL).get()))));
    if (b) {
...
BOOST_AUTO_TEST_CASE(variableRule_correctInput) {
  ...
  bool b{};
  const array outputs{
      string{"a"},     // variable of exactly one alphabetic character
      string{"a1b-_"}  // variable with all categories of accepted characters
  };
  for (const string& out : outputs) {
    const string inp{format("%{}%", out)};
    string s;
    BOOST_TEST_CONTEXT("for inp = `" << inp << '`') {
      b = false;
...
BOOST_AUTO_TEST_CASE(valueRule_correctInput) {
  ...
  double d{};
  SymbolsTable st;
  const array inputs{"100."s, ".1"s, "0.1"s, "1e2"s, "11E-2"s, "11.22e-2"s};
  for (const string& inp : inputs) {
    shared_ptr<const NumericExpr> ne;
    istringstream iss{inp};
    BOOST_TEST_CONTEXT("for inp = `" << inp << '`') {
      BOOST_CHECK(b = !(iss >> d).fail());
      if (b) {
        b = false;
        ...
        if (b) {
          ...
          if (b) {
            ...
            const auto& constValOpt{ne->constValue()};
            BOOST_CHECK(b = constValOpt.has_value());
            if (b)
              BOOST_CHECK_NO_THROW(BOOST_TEST(*constValOpt == d));
          }
        }
      }
    }
    ...
    BOOST_TEST_CONTEXT("for inp = `" << inp
      ...
      if (b) {
        ...
        if (b) {
          ...
          BOOST_CHECK(b = constValOpt.has_value());
          if (b)
            BOOST_CHECK_NO_THROW(BOOST_TEST(*constValOpt == expected));
        }
      }
    }
  }
    ...
    BOOST_TEST_CONTEXT("for inp = `" << inp
      ...
      if (b) {
        ...
        if (b) {
          ...
          BOOST_CHECK(b = constValOpt.has_value());
          if (b)
            BOOST_TEST(*constValOpt == expected);
        }
      }
    }
  }
...
BOOST_AUTO_TEST_CASE(valueSetRule_correctInput) {
  ...
  }
  {
    ...
    if (b) {
      ...
      if (b) {
        BOOST_CHECK(b = !vs.vors.at(0ULL).isRange());
        if (b)
          BOOST_CHECK_NO_THROW(BOOST_TEST(vs.vors.at(0ULL).value(st) == 14.));

        BOOST_CHECK(b = vs.vors.at(1ULL).isRange());
        if (b)
          BOOST_CHECK_NO_THROW({
            const auto range = vs.vors.at(1ULL).range(st);
            BOOST_TEST(range.first == 2.);
            BOOST_TEST(range.second == 2.);
          });

        BOOST_CHECK(b = vs.vors.at(2ULL).isRange());
        if (b)
          BOOST_CHECK_NO_THROW({
            const auto range = vs.vors.at(2ULL).range(st);
...

  {
    ...
    if (b) {
      ...
      if (b) {
        BOOST_CHECK(b = !vs.vors.at(0ULL).isRange());
        if (b)
          BOOST_CHECK_NO_THROW(BOOST_TEST(vs.vors.at(0ULL).value(st) == 5.));
...

  {
    ...
    if (b) {
      ...
      if (b) {
        ...
        const auto& constValOpt{le->constValue()};
        BOOST_CHECK(b = constValOpt.has_value());
        if (b)
          BOOST_CHECK(*constValOpt);
      }
    }
  }
    ...
    BOOST_TEST_CONTEXT("for inp = `" << inp << "` and expected = " << boolalpha
      ...
      if (b) {
        ...
        if (b) {
          ...
          BOOST_CHECK(b = constValOpt.has_value());
          if (b)
            BOOST_CHECK(*constValOpt == expected);
        }
      }
    }
  }
