:: To use the RiverCrossing.exe generated by MinGW/Cygwin/MSVC,
:: the server needs to be launched from the same type of environment with the
:: exception of MSVC builds, which can be launched also from MinGW/Cygwin environments.
:: That is:
:: - RiverCrossing.exe compiled by Cygwin must be launched only from Cygwin
:: - RiverCrossing.exe compiled by MinGW must be launched only from MinGW
...
:: All 3 environments (MinGW/Cygwin/MSVC) should create a single server
